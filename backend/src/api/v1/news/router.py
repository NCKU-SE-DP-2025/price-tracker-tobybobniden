...
async def get_news_articles(
    ...
    for article in articles:
        upvotes_count, is_upvoted = service.get_article_upvote_details(db, article.id, None)
        result.append({
            "id": article.id,
            "url": article.url,
            "title": article.title,
...
async def read_user_news(
    ...
    result = []
    for article in articles:
        upvotes, upvoted = service.get_article_upvote_details(db, article.id, user.id)
        result.append({
            "id": article.id,
            "url": article.url,
            "title": article.title,
...
async def upvote_news(
    ...
    """點贊新聞"""
    message = service.toggle_upvote(db, news_id, user.id)
